...
category: scenes
sub_category: ""  # для anti-examples — какая исходная категория (scenes/dialogues/etc)
example_type: positive  # positive | anti-example
...
[Если автор обсуждает, что в сцене НЕ работает — это материал для anti-example.]

# Что нужно от шоураннера

[Конкретные действия: создать golden-файл такой-то категории; уточнить такой-то аспект; отклонить как нерелевантное.]

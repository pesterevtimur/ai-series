...
Если шоураннер считает exit 1 false positive (логика скрипта багована, данные «архивные», ссылка «неважная»):
- **BLOCK не снимается без явной записи в `decisions/D-NNN-*.md`** с обоснованием.
- Альтернатива — починить данные (добавить отсутствующий артефакт ИЛИ убрать ссылку).
- «Игнорируй, это очевидно false positive» = bypass discipline, NOT acceptance.

### Свежесть результата

`consistency_check.py` дёшев (< 2 сек). **«Гонял минут N назад» не considered evidence** — re-run обязателен (см. `.claude/skills/evidence-before-action/SKILL.md` четыре проверки соответствия). Параллельные сессии или auto-format hooks могли изменить любой `references` за это время.
